...
//extern int BankBinArrayCount[];

static NSArray *bankBinArray = nil;

@implementation BinCodeSearch

//char* strBankBinArray[]
+(NSString*)getBankNameByBin:(char*)numbers count:(int)nCount
{
    NSUInteger len = sizeof(strBankBinArray)/sizeof(strBankBinArray[0]) ;//[BinCodeSearch BankBinArray].count;
    ...
    bankName = nil;
    for(i = 0; i < len; i+= 2){
//        NSString *tmp = [BinCodeSearch BankBinArray][i];
        const char *ch = strBankBinArray[i];//[tmp UTF8String];//[tmp getCharacters:<#(unichar *)#>];
        slen = strlen(strBankBinArray[i]);//tmp.length;
        ...
        if(bEqual == 1) {
            ...
            if(stdCardNumCount == 0 || stdCardNumCount == dlen)
                bankName = [NSString stringWithUTF8String:strBankBinArray[i+1]];//[BinCodeSearch BankBinArray][i+1];
            break;
        }
    }
...
+( NSArray *)BankBinArray
{
    //char* strBankBinArray[]
    if ( ! bankBinArray) {
        bankBinArray = [[NSArray alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"BankBinList" ofType:@"plist"]];
